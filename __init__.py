...
@cbpi.step
class BoilStepWithHopDropperAndPump(StepBase):
    '''
    ...
    hop_dropper_2 = StepProperty.Actor("Boil Pump", description="Please select the boil pump")
    hop_dropper_1 = StepProperty.Actor("Hop Dropper", description="Please select the hop dropper actor")
    ...
    hop_1_actor = Property.Select("Actor", ["Hop dropper", "Boil pump"], description="Please select the actor")
    ...
    hop_2_actor = Property.Select("Actor", ["Hop dropper", "Boil pump"], description="Please select the actor")
    ...
    hop_3_actor = Property.Select("Actor", ["Hop dropper", "Boil pump"], description="Please select the actor")
    ...
    hop_4_actor = Property.Select("Actor", ["Hop dropper", "Boil pump"], description="Please select the actor")
    ...
    hop_5_actor = Property.Select("Actor", ["Hop dropper", "Boil pump"], description="Please select the actor")
    ...
    hop_6_actor = Property.Select("Actor", ["Hop dropper", "Boil pump"], description="Please select the actor")
    ...
    hop_7_actor = Property.Select("Actor", ["Hop dropper", "Boil pump"], description="Please select the actor")
    ...
    def check_hop_timer(self, number, value):
      try:
          if self.__getattribute__("hop_%s_added" % number) is not True and self.timer_end - int(time.time()) <= (int(value) * 60):
              ...
              if self.__getattribute__("hop_%s_actor" % number) == "Hop dropper":
                  if self.hop_dropper_1 is not None:
                      self.actor_on(int(self.hop_dropper_1))
                  self.notify("Hop Alert", "Adding Hops #%s: %s" % (number, self.__getattribute__("hop_%s_desc" % number)), timeout=10000)
              elif self.__getattribute__("hop_%s_actor" % number) == "Boil pump":
                  if self.hop_dropper_2 is not None:
                      self.actor_on(int(self.hop_dropper_2))
                  self.notify("Pump Alert", "Boil pump activated", timeout=10000)
